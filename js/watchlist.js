...
if (localStorage.getItem("savedMovies")){
    watchlist = JSON.parse(localStorage.getItem("savedMovies"))
    console.log(watchlist)
    renderStorage(watchlist)

}

// render watchlist on DOM

function renderStorage (i){
    ...
    if (i.length === 0){
        html.innerHTML = `
            <div class="empty-watchlist">
                ...
                <span>
                    <a href="index.html">
                        <svg class="watchlistIcon" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="grid-card">
                ...
                <div class="movie-card">
                    <div class="movie-title">
                        ...
                        <p class="ratingLabel">${data.imdbRating}</p>
                    </div>

                    <div class="movie-info">
                        <p class="minutes-label">${data.Runtime}</p>
                        <p class="genre-label">${data.Genre}</p>
                        ...
                        <p class="watchlist-label" data-id="${data.imdbID}"role="button">Remove</p>
                    </div> 
...
// remove saved move from watchlist

document.querySelector("main").addEventListener("click", (e) =>{

    if (e.target.classList.contains("watchlist-label")){
        const movieId = e.target.dataset.id
        console.log("the one to delete", movieId)
        removeMovie(movieId)
...
function removeMovie(movie){
    ...
    const filteredMovies = watchlist.filter(function(data){

        return data.imdbID !== movie
